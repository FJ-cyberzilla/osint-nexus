...
class TestCLIMain(unittest.TestCase):
    ...
    def test_main_scan(self, mock_logging, mock_handle_scan, mock_parse_args):
        # Setup mock args
        args = argparse.Namespace(command="scan", debug=False)
        mock_parse_args.return_value = args

        main()

        mock_handle_scan.assert_called_once_with(args=args)
    ...
    def test_main_health(self, mock_logging, mock_handle_health, mock_parse_args):
        # Setup mock args
        args = argparse.Namespace(command="health", debug=False)
        mock_parse_args.return_value = args

        main()

        mock_handle_health.assert_called_once_with(args=args)
    ...
    def test_main_db_info(self, mock_logging, mock_handle_db_info, mock_parse_args):
        # Setup mock args
        args = argparse.Namespace(command="db-info", debug=False)
        mock_parse_args.return_value = args

        main()

        mock_handle_db_info.assert_called_once_with(args=args)

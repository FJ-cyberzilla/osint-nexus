...
async def test_infer_by_ports_empty_and_unknown():
    ...
    assert result_empty.possible_os == ["Unknown OS"]
    assert result_empty.detected_roles == ["No clear exposed roles"]
    assert result_empty.confidence_score == 0

    # Unknown ports
    result_unknown = await service.infer_by_ports("127.0.0.1", [12345, 67890])
    assert result_unknown.possible_os == ["Unknown OS"]
    assert result_unknown.detected_roles == ["No clear exposed roles"]

...
class VisualIntelligence(BaseModel):
    ...
    profile_picture: Optional[HttpUrl] = Field(default=None, description="URL to the target's avatar")
    banner_image: Optional[HttpUrl] = Field(default=None, description="URL to the background/banner")
    ...
    ocr_text: Optional[str] = Field(default=None, description="Text extracted from images via OCR")
...
class IntelligenceObject(BaseModel):
    ...
    id: uuid.UUID = Field(
        ...
        description="Unique identifier for this intelligence record"
    )
    scraped_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Time the intelligence was gathered (UTC)"
    )
    ...
    username: str = Field(
        ..., 
        min_length=1,
        description="The queried username or handle"
    )

    # --- Results ---
    found: bool = Field(
        ..., 
        description="True if the target was positively identified"
    )
    dork: str = Field(
        default="",
        description="Google-style dork query for this platform"
    )
    confidence: float = Field(
        default=0.0, 
        ge=0.0, 
        le=1.0, 
        description="Probability score of exact match (0.0 to 1.0)"
    )
